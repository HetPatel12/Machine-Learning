...
def backward_softmax(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    scores = forward_softmax(x).reshape(-1, 1)
    dy_dx = -np.matmul(scores, scores.T) + np.identity(x.shape[0])*(scores)
    return dy_dx.dot(grad_outputs)
    # *** END CODE HERE ***
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    for x in range(input_width - conv_width + 1):
        for y in range(input_height - conv_height + 1):
            for output_channel in range(conv_channels):
                dW[output_channel, :, :, :] += output_grad[output_channel, x, y] * data[:, x:(x + conv_width), y:(y + conv_height)]
                dd[:, x:(x + conv_width), y:(y + conv_height)] += output_grad[output_channel, x, y] * conv_W[output_channel, :, :, :]
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    for i in range(input_channels):
        for x in range(0, input_width, pool_width):
            for y in range(0, input_height, pool_height):
                temp = data[i, x:(x + pool_width), y:(y + pool_height)]
                dd[i, x:(x + pool_width), y:(y + pool_height)][i, np.unravel_index(temp.argmax(), temp.shape)] = output_grad[i, x // pool_width, y // pool_height]
...
def backward_prop(data, labels, params):
    ...
    dy_hat = backward_cross_entropy_loss(y, labels)
    dlogits = backward_softmax(logits, dy_hat)
    grads["W2"], grads["b2"], dflattened = backward_linear(W2, b2, flattened, dlogits)
    drelu = backward_relu(first_max_pool, dflattened.reshape(first_max_pool.shape))
    dmax_pool = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, drelu)
    grads["W1"], grads["b1"], _ = backward_convolution(W1, b1, data, dmax_pool)
