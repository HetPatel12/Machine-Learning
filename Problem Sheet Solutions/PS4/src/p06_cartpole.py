...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    transition_probs = mdp_data['transition_probs'][state] #a row of probabilities
    V = mdp_data['value'] #current future payoffs of each state
    expected = V.dot(transition_probs)
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    if reward == -1:
        mdp_data['reward_counts'][new_state, 0] += 1 # #times -1 reward
    mdp_data['reward_counts'][new_state, 1] += 1 # #times new_state reached
    # *** END CODE HERE ***
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    total_counts = np.sum(transition_counts, axis=1)
    num_states = transition_counts.shape[0]
    
    for state in range(num_states):
        for a in range(2):
            if total_counts[state][a] != 0:
                mdp_data['transition_probs'][state][:, a] = transition_counts[state][:, a]/total_counts[state, a] #update as given in question
    
    reward_counts = mdp_data['reward_counts']
    for state in range(num_states):
        sum_rewards = reward_counts[state][1]
        if sum_rewards != 0:
            mdp_data['reward'][state] = -reward_counts[state][0]/sum_rewards #update as given in question
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        curr_v = mdp_data['value']
        
        new_v = mdp_data['reward'] + gamma * np.max(curr_v.dot(transition_probs), axis=1)
            
        mdp_data['value'] = new_v
            
        iters += 1
        
        if (np.max(np.abs(new_v - curr_v)) < tolerance):
            break
