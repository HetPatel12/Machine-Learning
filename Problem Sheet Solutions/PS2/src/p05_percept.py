...
def predict(state, kernel, x_i):
    ...
    """
    # *** START CODE HERE ***
    # pred = sign(kernel(np.array(state[1]), x_i))
    # pred = np.sum(np.array(state[0])*pred)
    # return pred
    pred = 0
    for i in range(len(state[0])):
        pred += state[0][i]*(kernel(state[1][i], x_i))
    pred = sign(pred)
    return pred
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    #     fn = 0
    # else:
    #     fn = sign(kernel(np.array(state[1]), x_i))
    #     fn = np.sum(np.array(state[0])*fn)
    
    # state[0].append(learning_rate*(y_i - fn))
    # state[1].append(x_i)
    
    fn = 0
    if len(state[0]) != 0:
        for i in range(len(state[0])):
            fn += state[0][i]*(kernel(state[1][i], x_i))
        fn = sign(fn)
